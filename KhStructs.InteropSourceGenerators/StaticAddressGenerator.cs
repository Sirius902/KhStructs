...
internal sealed class StaticAddressGenerator : IIncrementalGenerator {
    private const string AttributeName = "KhStructs.Interop.Attributes.StaticAddressAttribute";
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context) {
        ...
        // group by struct
        IncrementalValuesProvider<(Validation<DiagnosticInfo, StructInfo> StructInfo,
                Validation<DiagnosticInfo, Seq<StaticAddressInfo>> StaticAddressInfos)>
            groupedStructInfoWithStaticAddressInfos =
                structAndStaticAddressInfos.TupleGroupByValidation();

        // make sure caching is working
        IncrementalValuesProvider<Validation<DiagnosticInfo, StructWithStaticAddressInfos>>
            structWithStaticAddressInfos =
        ...
        context.RegisterSourceOutput(structWithStaticAddressInfos, (sourceContext, item) => {
            item.Match(
                ...
                Succ: structWithStaticAddressInfo => {
                    sourceContext.AddSource(structWithStaticAddressInfo.GetFileName(),
                        structWithStaticAddressInfo.RenderSource());
                });
        });
    ...
    internal sealed record StaticAddressInfo(MethodInfo MethodInfo, SignatureInfo SignatureInfo, int Offset,
        ...
        public static Validation<DiagnosticInfo, StaticAddressInfo> GetFromRoslyn(
            ...
            Validation<DiagnosticInfo, int> validInstructionSize =
                staticAddressAttribute.GetValidAttributeArgument<int>("InstructionSize", 3, AttributeName,
                    methodSymbol);

            return (validMethodInfo, validSignature, validOffset, validIsPointer, validInstructionSize).Apply(
                (methodInfo, signature, offset, isPointer, instructionSize) =>
        ...
        public void RenderPointer(IndentedStringBuilder builder, StructInfo structInfo) {
            ...
            builder.AppendLine(
                $"public static {MethodInfo.ReturnType}{pointerText}p{MethodInfo.Name} => ({MethodInfo.ReturnType}{pointer}){structInfo.Name}.Addresses.{MethodInfo.Name}.Value;");
        }
